...
void OLED_Refresh_Gram(void)
{
	...
	{  
	    ...
	    int ret = X_WriteMulti(I2C_NUM_1,0x3C,0x40,SSD1306_WIDTH, &OLED_GRAM[i][0]);
                if (ret == ESP_FAIL) {
                  printf("I2C Fail\n");
                } 
	} 
...
{	
   ...
   int ret = X_WrByte(I2C_NUM_1,0x3C,index,dat); 
   if (ret == ESP_FAIL) {
            printf("I2C Fail\n");
        }
} 
...
{      			    
    ...
    for(t=0;t<csize;t++)
    {   
		if(size==12)temp=asc2_1206[(uint8_t)chr][t]; 	 	//调用1206字体
		else if(size==16)temp=asc2_1608[(uint8_t)chr][t];	//调用1608字体
		else if(size==24)temp=asc2_2412[(uint8_t)chr][t];	//调用2412字体
		else return;								//没有的字库
        for(t1=0;t1<8;t1++)
		{
